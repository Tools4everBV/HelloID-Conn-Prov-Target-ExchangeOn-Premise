...
try {
    if($dryRun -eq $false){
        ...
        # Revoke Exchange Mailbox permission
                ...
                foreach($permission in $pRef.Permissions){
                    switch($permission){
                        ...
                        "Send As" {
                            ...
                            # No error is thrown when user already has permission
                            $removeSAPermission = Remove-AdPermission -Identity $pRef.id -ExtendedRights "Send As"-User $aRef.Guid -Confirm:$false -ErrorAction Stop
