#################################################
# HelloID-Conn-Prov-Target-Exchange-On-Premise-Disable
# PowerShell V2
#################################################
    ...
    switch ($action) {
        'DisableAccount' {
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Disabling Exchange-On-Premise account with accountReference: [$($actionContext.References.Account)]"
                
                try {
                    Invoke-Command -Session $remoteSession -ScriptBlock {
                        $aref = $using:actionContext.References.Account
                        Set-Mailbox -Identity $aref -HiddenFromAddressListsEnabled $True
                    } -ErrorAction Stop                    
                }
                catch { 
                    throw $_                        
                ...
                Write-Information "[DryRun] Disable Exchange-On-Premise account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }
            $outputContext.Success = $true
            $outputContext.AuditLogs.Add([PSCustomObject]@{
                    Message = 'Disable mailbox was successful'
                    IsError = $false
                })
            break
        }
