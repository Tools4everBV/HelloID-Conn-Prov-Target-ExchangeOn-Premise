            ...
            try {
                ...
                foreach ($permissiontype in $permission.Permissions) {
                    switch ($permissiontype) {
                        "Full Access" {
                            [Void][Void]$verboseLogs.Add("Granting permission FullAccess to mailbox $($permission.DisplayName) ($($permission.Reference)) for user $($person.DisplayName) ($($account))")
                            ...
                            [Void]$informationLogs.Add("Successfully granted permission FullAccess to mailbox $($permission.DisplayName) ($($permission.Reference)) for user $($person.DisplayName) ($($account))")
                            $auditLogs.Add([PSCustomObject]@{
                                    Action  = "GrantPermission"
                                    Message = "Successfully set full access permission to mailbox ($($permission.DisplayName)) for user ($($person.DisplayName))"
                                    IsError = $false
                                })
                        }
                        "Send As" {
                            [Void]$verboseLogs.Add("Granting permission SendAs to mailbox $($permission.DisplayName) ($($permission.Reference)) for user $($person.DisplayName) ($($account))")
                            # No error is thrown when user already has permission
                            $null = Add-AdPermission -Identity $permission.Reference -ExtendedRights "Send As"-User $account -Confirm:$false -ErrorAction Stop
                            [Void]$informationLogs.Add("Successfully granted permission SendAs to mailbox $($permission.DisplayName) ($($permission.Reference)) for user $($person.DisplayName) ($($account))")
                            $auditLogs.Add([PSCustomObject]@{
                                    Action  = "GrantPermission"
                                    Message = "Successfully set send as permission to mailbox ($($permission.DisplayName)) for user ($($person.DisplayName))"
                                    IsError = $false
                                })
                        }
                        "Send on Behalf" {
                            [Void]$verboseLogs.Add("Granting permission SendonBehalf to mailbox $($permission.DisplayName) ($($permission.Reference)) for user $($person.DisplayName) ($($account))")
                            # No error is thrown when user already has permission
                            ...
                            [Void]$informationLogs.Add("Successfully granted permission SendonBehalf to mailbox $($permission.DisplayName) ($($permission.Reference)) for user $($person.DisplayName) ($($account))")
                            $auditLogs.Add([PSCustomObject]@{
                                    Action  = "GrantPermission"
                                    Message = "Successfully set send on behalf permission to mailbox ($($permission.DisplayName)) for user ($($person.DisplayName))"
                                    IsError = $false
                                })
                        }
                    }
                ...
                $auditLogs.Add([PSCustomObject]@{
                        Action  = "GrantPermission"
                        Message = "Successfully granted permission $($permission.Permissions -join " & ") to mailbox $($permission.DisplayName) ($($permission.Reference)) for user $($person.DisplayName) ($($account))"
                        IsError = $false
                    }
                )      
            }
                ...
                elseif ($_ -like "*User or group ""$($account)"" wasn't found*") {
                    ...
                    [Void]$warningLogs.Add("Error granting permission $($permission.Permissions -join " & ") to mailbox $($permission.DisplayName) ($($permission.Reference)) for user $($person.DisplayName) ($($account)). Error: $_")
                    $success = $false
                    $auditLogs.Add([PSCustomObject]@{
                            Action  = "GrantPermission"
                            Message = "Failed to grant permission $($permission.Permissions -join " & ") to mailbox $($permission.DisplayName) ($($permission.Reference)) for user $($person.DisplayName) ($($account))"
                            IsError = $true
                        }
                    )
                }
