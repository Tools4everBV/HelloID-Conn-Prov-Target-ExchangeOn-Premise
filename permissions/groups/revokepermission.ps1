#############################################################################
# HelloID-Conn-Prov-Target-Exchange-Server-On-Premises-RevokePermission-Group
# PowerShell V2
#############################################################################
# Enable TLS1.2
...
try {
    ...
    if (-not($actionContext.DryRun -eq $true)) {
        ...
        Write-Information "Revoking Exchange Server On Premises permission: [$($actionContext.References.Permission.DisplayName)] - [$($actionContext.References.Permission.Reference)]"
        # Make sure to test with special characters and if needed; add utf8 encoding.
        # Grant Exchange Groupmembership
        $removeExchangeGroupMembership = Invoke-Command -Session $remoteSession -ScriptBlock {
            try {
                ...
                [Void]$verboseLogs.Add("Revoking permission $($permission.DisplayName) ($($permission.Reference)) from $($account.UserPrincipalName) ($($account.Guid))")
                $null = Remove-DistributionGroupMember -Identity $permission.Reference -Member $account.Guid -BypassSecurityGroupManagerCheck:$true -Confirm:$false -ErrorAction Stop
                [Void]$informationLogs.Add("Successfully revoked permission $($permission.DisplayName) ($($permission.Reference)) from $($account.UserPrincipalName) ($($account.Guid))")

                $success = $true
                $auditLogs.Add([PSCustomObject]@{
                        Action  = "RevokePermission"
                        Message = "Successfully revoked permission $($permission.DisplayName) ($($permission.Reference)) from $($account.UserPrincipalName) ($($account.Guid))"
                        IsError = $false
                    }
                )      
            }
                ...
                if ($_ -like "*isn't a member of the group*") {
                    [Void]$warningLogs.Add("The recipient $($account.UserPrincipalName) ($($account.Guid)) is not a member of the group $($permission.DisplayName) ($($permission.Reference))")
                    $success = $true
                    $auditLogs.Add([PSCustomObject]@{
                            Action  = "RevokePermission"
                            Message = "Successfully revoked permission $($permission.DisplayName) ($($permission.Reference)) from $($account.UserPrincipalName) ($($account.Guid))"
                ...
                elseif ($_ -like "*Couldn't find object ""$($account.Guid)""*") {
                    ...
                    [Void]$warningLogs.Add("Error revoking permission $($permission.DisplayName) ($($permission.Reference)) from $($account.UserPrincipalName) ($($account.Guid)). Error: $_")
                    $success = $false
                    $auditLogs.Add([PSCustomObject]@{
                            Action  = "RevokePermission"
                            Message = "Failed to revoke permission $($permission.DisplayName) ($($permission.Reference)) from $($account.UserPrincipalName) ($($account.Guid))"
                            IsError = $true
                        }
                    )
                }
        ...
        $success = $removeExchangeGroupMembership.success
        $auditLogs = $removeExchangeGroupMembership.auditLogs

        # Log the data from logging arrarys (since the "normal" Write-Information isn't sent to HelloID as another PS session performs the commands)
        $verboseLogs = $removeExchangeGroupMembership.verboseLogs
        foreach ($verboseLog in $verboseLogs) { Write-Verbose -Verbose $verboseLog }
        $informationLogs = $removeExchangeGroupMembership.informationLogs
        foreach ($informationLog in $informationLogs) { Write-Information $informationLog }
        $warningLogs = $removeExchangeGroupMembership.warningLogs
        foreach ($warningLog in $warningLogs) { Write-Warning $warningLog }
        $errorLogs = $removeExchangeGroupMembership.errorLogs
        foreach ($errorLog in $errorLogs) { Write-Warning $errorLog }
